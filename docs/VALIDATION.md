...
## Companion skill and persistent local setup

The companion skill passes the skill-creator validator, reference-link checks and a separate privacy review. Codex's app-server discovery API recognizes the installed copy as one enabled user-scoped skill. The installed 0.2.0 release also passes `doctor`, discovery of its 29 MCP tools, and a browser startup check without page errors. The normal user MCP registration uses the installed executable and a persistent workspace outside the source repository.

A fresh Codex CLI 0.154.0 session explicitly loaded the installed skill and its references in a separate test workspace. Given original generic CSS and a short design brief, it imported and saved a design system, created a native editable one-pager, inspected actual PNG pixels, corrected spacing with a targeted edit, rerendered revision 3 and exported a PDF. The session exited successfully; runtime configuration and raw host logs are kept outside the public skill. This is evidence for the companion workflow, not a new Claude Design account-export compatibility claim.
